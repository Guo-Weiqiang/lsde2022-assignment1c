...
def cruncher(datadir, a1, a2, a3, a4, d1, d2): 
    ...
    con.execute("CREATE TEMP TABLE scores AS \
                SELECT person.personId, person.locatedIn, person.birthday, count(person.personId) AS score, min(focus.nofan) AS nofan FROM person, focus \
                WHERE person.personId = focus.personId \
                GROUP BY person.personId, person.locatedIn, person.birthday")


    # candas
    con.execute("CREATE TEMP TABLE cands AS\
                SELECT * FROM scores WHERE score > 0 AND nofan = TRUE \
                AND %d <= month(birthday) * 100 + day(birthday)\
                AND month(birthday) * 100 + day(birthday) <= %d" % (d1, d2))
...
def run_cruncher(datadir, query_file_path, results_file_path, number_of_queries = 10):
    ...
    con.execute("CREATE TABLE person AS \
                SELECT * FROM '/opt/lsde/dataset-sf100-csvs/person_parquet/*.parquet'")
    con.execute("CREATE TABLE knows AS \
                SELECT * FROM '/opt/lsde/dataset-sf100-csvs/knows_parquet/*.parquet' ")
    con.execute("CREATE TABLE interests AS \
                SELECT * FROM '/opt/lsde/dataset-sf100-csvs/interest_parquet/*.parquet' ")
